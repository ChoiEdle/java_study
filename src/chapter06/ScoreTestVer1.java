...
public class ScoreTestVer1 {
	...
	public static void main(String[] args) {
		...
		String[] subjectList = {"국어", "영어", "수학", "총점", "평균"};
		int tot = 0;
		int avg = 0;
		...
		for(int i=0; i<scoreList.length; i++) {
			if(i<scoreList.length-2) {
				...
				tot += scoreList[i];
				avg = tot/(scoreList.length-2);
			}else {
				scoreList[scoreList.length-2] = tot;
				scoreList[scoreList.length-1] = avg;
				i = scoreList.length;	//break; 역할
			}
		}
...
//		scoreList[2] = scan.nextInt();
		...
		for(int score : scoreList) System.out.print(score + "\t");
